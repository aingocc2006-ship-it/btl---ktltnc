...
public:
    ...
    void loadStudents(const string& fileName)
    {
        ...
        while (getline(fin, line))
        {
            ...
            SinhVien s;
            int idx = 0;
            s.setId(safeStoi(tk[idx++]));           // tk[0]: id
            // skip tk[1]: stt (sequence number)
            idx++;
            s.setUsername(tk[idx++]);               // tk[2]: username
            s.setPassword(tk[idx++]);               // tk[3]: password
            s.setFullName(tk[idx++]);               // tk[4]: fullName
            s.setlopChuNhiem(tk[idx++]);            // tk[5]: class
            s.setGioiTinh(tk[idx++]);               // tk[6]: gender
            s.setBirthDay(tk[idx++]);               // tk[7]: birthDate
            s.setSdt(tk[idx++]);                    // tk[8]: phone
            s.setDiemTB(safeStod(tk[idx++]));       // tk[9]: avgScore
            s.setTeacherId(safeStoi(tk[idx++]));    // tk[10]: teacherId
            if (idx < (int)tk.size())
            {
