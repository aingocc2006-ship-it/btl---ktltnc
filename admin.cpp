...
struct deThi {
    ...
    time_t ketThuc;
    bool locked;
    vector<string> dsLopDuocThi; // Lưu danh sách tên lớp được phép thi

    deThi() : id(0), khoa(0), tenMon(""), thoiGianLamBai(0), loaiThi(1), phongThiId(0), batDau(0), ketThuc(0), locked(false) {}
    ...
    bool isAvailable() const {
        ...
        if (batDau == 0 || ketThuc == 0) return true;
        time_t now = time(0);
...
struct ClassInfo {
    int id;
    string className;
    int coVanHocTapId;              // Thay cho teacherId cũ (1 Cố vấn học tập)
    vector<int> danhSachGiangVien;  // Danh sách nhiều giáo viên giảng dạy
    int studentCount;
    int teacherId;
};
struct PhongThi {
...
public:
    ...
    void thietLapDeThi() {
        ...
        do {
            ...
            if (de.ketThuc == 0 || de.ketThuc <= de.batDau) {
                cout << "Dinh dang khong hop le hoac ket thuc truoc bat dau. Thu lai!\n";
                de.ketThuc = 0;
            }
        } while (de.ketThuc == 0);
    ...
    void xemLichThi() {
        if (maDe.empty()) { cout << "Chua co de thi de hien thi lich.\n"; return; }
        SetColor(CYAN);
        cout << "\n===== LICH THI =====\n";
        SetColor(WHITE);
    ...
    void menu() {
        ...
        do {
            ...
            case 4:
                ...
                break;
            case 5: khoaMoDeThi(); break;
            case 6: xemLichThi(); break;
            }
        } while (ch != 0);
    }
};
...
void QuanLyThi::saveDeThi() const {
    ...
    for (const auto& de : maDe) {
        fout << de.id << '|' << de.tenMon << '|' << de.thoiGianLamBai << '|' << de.loaiThi << '|' << de.danhSachCauHoi.size()
            << '|' << de.phongThiId << '|' << formatDateTime(de.batDau) << '|' << formatDateTime(de.ketThuc) << '|' << (de.locked ? 1 : 0) << '|';

        for (size_t i = 0; i < de.dsLopDuocThi.size(); i++) {
            fout << de.dsLopDuocThi[i] << (i < de.dsLopDuocThi.size() - 1 ? "," : "");
        }
        if (de.dsLopDuocThi.empty()) fout << "ALL";
        fout << '\n';
...
void QuanLyThi::loadDeThi() {
    ...
    while (getline(fin, line)) {
        ...
        deThi de;
        string idStr, tgStr, loaiStr, soCauStr, phongThiStr, batDauStr, ketThucStr, lockedStr, dsLopStr;

        getline(ss, idStr, '|'); getline(ss, de.tenMon, '|'); getline(ss, tgStr, '|');
        getline(ss, loaiStr, '|'); getline(ss, soCauStr, '|'); getline(ss, phongThiStr, '|');
        getline(ss, batDauStr, '|'); getline(ss, ketThucStr, '|'); getline(ss, lockedStr, '|');
        getline(ss, dsLopStr);

        de.id = stoi(idStr); de.thoiGianLamBai = stoi(tgStr); de.loaiThi = stoi(loaiStr);
        de.phongThiId = phongThiStr.empty() ? 0 : stoi(phongThiStr);
        de.batDau = batDauStr.empty() ? 0 : parseDateTime(batDauStr);
        de.ketThuc = ketThucStr.empty() ? 0 : parseDateTime(ketThucStr);
        de.locked = (lockedStr == "1");

        if (dsLopStr != "ALL" && !dsLopStr.empty()) {
            stringstream ssLop(dsLopStr); string tenLop;
            while (getline(ssLop, tenLop, ',')) { de.dsLopDuocThi.push_back(tenLop); }
        }

        int soCau = stoi(soCauStr);
        for (int i = 0; i < soCau; i++) {
            getline(fin, line); stringstream s2(line); CauHoi ch; string idCH, dapAnDungStr;
            getline(s2, idCH, '|'); getline(s2, ch.noiDung, '|');
            getline(s2, ch.dapAn[0], '|'); getline(s2, ch.dapAn[1], '|');
            getline(s2, ch.dapAn[2], '|'); getline(s2, ch.dapAn[3], '|'); getline(s2, dapAnDungStr);
            ch.id = stoi(idCH); ch.dapAnDung = stoi(dapAnDungStr); de.danhSachCauHoi.push_back(ch);
        }
        maDe.push_back(de);
    }
    fin.close();
...
public:
    ...
    bool kiemTraLopDuocThi(const deThi& de) const {
        if (de.dsLopDuocThi.empty()) return true; // Áp dụng toàn trường
        for (const string& l : de.dsLopDuocThi) {
            if (l == this->lop) return true;
        }
        return false;
    }
    ...
    void xemDanhSachOnTap(QuanLyThi& thi) {
        ...
        for (const auto& de : thi.getMaDe()) {
            if (!kiemTraLopDuocThi(de)) continue;
            if (de.loaiThi == 2) {
    ...
    void lamBaiThi(QuanLyThi& thi) {
        ...
        if (!kiemTraLopDuocThi(*deHienTai)) {
            SetColor(RED);
            cout << "Ban khong thuoc danh sach lop duoc phep lam bai thi nay!\n";
            SetColor(WHITE);
            return;
        }        if (deHienTai->locked) {
            SetColor(RED);
            cout << "De thi hien dang bi khoa. Vui long lien he giang vien hoac admin.\n";
            SetColor(WHITE);
            return;
        }

        if (!deHienTai->isAvailable()) {
            SetColor(YELLOW);
            cout << "De thi hien chua hoat dong. Lich thi: " << deHienTai->getSchedule() << "\n";
            SetColor(WHITE);
            return;
        }
...
public:
    ...
    void addClass() {
        ...
        c.coVanHocTapId = nhapSo<int>("Nhap ID Co van hoc tap (0 neu chua gan): ");
        if (c.coVanHocTapId != 0 && !teacherIdExists(c.coVanHocTapId)) { cout << "Giang vien khong ton tai.\n"; return; }

        int soGV = nhapSo<int>("Nhap so luong giao vien giang day lop nay: ");
        for (int i = 0; i < soGV; i++) {
            int idGV = nhapSo<int>("Nhap ID giao vien thu " + to_string(i + 1) + ": ");
            if (teacherIdExists(idGV)) {
                c.danhSachGiangVien.push_back(idGV);
            }
            else {
                SetColor(YELLOW); cout << "ID Giang vien khong ton tai. Da bo qua.\n"; SetColor(WHITE);
            }
        }
    ...
    void viewClasses() const {
        ...
        for (int i = 0; i < (int)classes.size(); i++) {
            cout << "STT: " << i + 1 << " | ID lop: " << classes[i].id
                << " | Ten lop: " << classes[i].className
                << " | CVHT ID: " << classes[i].coVanHocTapId
                << " | So GV day: " << classes[i].danhSachGiangVien.size()
                << " | So SV: " << classes[i].studentCount << '\n';
        }
    }
    void editClass() {
        cout << "\n===== SUA LOP =====\n";
    ...
    void addRoom() {
        ...
        if (roomIdExists(r.id)) { cout << "ID phong thi da ton tai.\n"; return; }
    ...
    void viewRooms() const {
        cout << "\n===== DANH SACH PHONG THI =====\n";
        if (examRooms.empty()) { cout << "Chua co phong thi nao.\n"; return; }
        for (int i = 0; i < (int)examRooms.size(); i++) {
    ...
    void editRoom() {
        ...
        if (idx == -1) { cout << "Khong tim thay phong thi.\n"; return; }
    ...
    void exportExamReport(const QuanLyThi& thi) const {
        ...
        fout << "ID_SV,Ten_SV,Ma_De,Mon,Loai_Thi,Điểm,Thoi_gian,Phong,Thoi_gian_bat_dau,Thoi_gian_ket_thuc,Trang_thai\n";
        for (const auto& hs : students) {
            for (const auto& kq : hs.getDanhSachKetQua()) {
                const deThi* de = thi.timDeThi(kq.maDe);
                string phong = de ? to_string(de->phongThiId) : "0";
                string batDau = de ? formatDateTime(de->batDau) : "N/A";
                string ketThuc = de ? formatDateTime(de->ketThuc) : "N/A";
                string trangThai = de ? (de->locked ? "Khoa" : (de->isAvailable() ? "Dang mo" : "Chua den gio/da ket thuc")) : "Khong co";
                fout << hs.getId() << ',' << hs.getFullName() << ',' << kq.maDe << ',' << kq.mon << ',' << kq.loaiThi << ',' << kq.diem << ',' << kq.thoiGian << ','
                    << phong << ',' << batDau << ',' << ketThuc << ',' << trangThai << '\n';
            }
        }
        fout.close();
        cout << "Xuat bao cao thi thanh cong: bao_cao_thi.csv\n";
    }
    ...
    void viewExamStatistics(const QuanLyThi& thi) const {
        ...
        if (examScores.empty()) { cout << "Chua co ket qua thi de thong ke.\n"; return; }
        for (const auto& exam : examScores) {
            const deThi* de = thi.timDeThi(exam.first);
            if (!de) continue;
            const auto& scores = exam.second;
            double sum = 0; double maxScore = 0; double minScore = 10;
            for (double v : scores) { sum += v; maxScore = max(maxScore, v); minScore = min(minScore, v); }
            double avg = sum / scores.size();
            cout << "Ma de: " << de->id << " | Mon: " << de->tenMon << " | So luot thi: " << scores.size() << " | Diem TB: " << avg << " | Min: " << minScore << " | Max: " << maxScore << '\n';
    ...
    void loadRooms(const string& fileName) {
        ...
        if (!fin) { cout << "Khong mo duoc file phong thi!\n"; return; }
        string line;
        while (getline(fin, line)) {
            if (line.empty()) continue;
            stringstream ss(line);
            string idStr, tenPhong, sucChuaStr, lockedStr;
            getline(ss, idStr, '|'); getline(ss, tenPhong, '|'); getline(ss, sucChuaStr, '|'); getline(ss, lockedStr);
            PhongThi r;
            r.id = stoi(idStr); r.tenPhong = tenPhong; r.sucChua = stoi(sucChuaStr); r.locked = (lockedStr == "1");
            examRooms.push_back(r);
        }
        fin.close();
    ...
    void viewSystemStatistics() const {
        ...
        cout << "So phong thi: " << examRooms.size() << '\n';
    ...
    void saveStudents() const {
        ...
        for (const auto& s : students) {
            file << s.getId() << '|' << s.getUsername() << '|' << s.getPassword() << '|' << s.getFullName() << '|' << s.getLop() << '|' << s.getGioiTinh() << '|' << s.getBirthDay() << '|' << s.getSdt() << '|' << s.layDiem() << '|' << s.layMaGV() << '\n';
        }
        file.close();
    }
    ...
    void saveClasses() const {
        ...
        for (const auto& c : classes) {
            fout << c.id << '|' << c.className << '|' << c.coVanHocTapId << '|' << c.studentCount << '|';
            for (size_t i = 0; i < c.danhSachGiangVien.size(); i++) {
                fout << c.danhSachGiangVien[i] << (i < c.danhSachGiangVien.size() - 1 ? "," : "");
            }
            if (c.danhSachGiangVien.empty()) fout << "0";
            fout << '\n';
        }
        fout.close();
    }
    ...
    void loadClasses(const string& fileName) {
        ...
        while (getline(fin, line)) {
            if (line.empty()) continue;
            stringstream ss(line);
            string idStr, className, cvhtIdStr, studentCountStr, gvListStr;

            getline(ss, idStr, '|'); getline(ss, className, '|');
            getline(ss, cvhtIdStr, '|'); getline(ss, studentCountStr, '|');
            getline(ss, gvListStr);

            ClassInfo c;
            c.id = stoi(idStr); c.className = className;
            c.coVanHocTapId = stoi(cvhtIdStr); c.studentCount = stoi(studentCountStr);

            if (!gvListStr.empty() && gvListStr != "0") {
                stringstream ssGV(gvListStr); string gvId;
                while (getline(ssGV, gvId, ',')) {
                    if (!gvId.empty()) c.danhSachGiangVien.push_back(stoi(gvId));
                }
            }
            classes.push_back(c);
        }
    ...
    void searchStudent() {
        cout << "\n===== TIM KIEM SINH VIEN =====\n";
        cout << "1. Tim theo ID\n2. Tim theo ten\n3. Tim theo lop\n";
        int option = nhapSo<int>("Chon tieu chi: ");

        if (option == 1) {
            int id = nhapSo<int>("Nhap ID: ");
            bool found = false;
            for (const auto& s : students) {
                if (s.getId() == id) {
                    cout << "ID: " << s.getId() << " | Ten: " << s.getFullName() << " | Lop: " << s.getLop() << " | Diem TB: " << s.layDiem() << '\n';
                    found = true;
                    break;
                }
            }
            ...
            string name = inputLine("Nhap ten (phan hay day du): ");
            bool found = false;
            for (const auto& s : students) {
                if (s.getFullName().find(name) != string::npos) {
                    cout << "ID: " << s.getId() << " | Ten: " << s.getFullName() << " | Lop: " << s.getLop() << " | Diem TB: " << s.layDiem() << '\n';
                    found = true;
                }
            }
            ...
            string lop = inputLine("Nhap lop: ");
            bool found = false;
            for (const auto& s : students) {
                if (s.getLop() == lop) {
                    cout << "ID: " << s.getId() << " | Ten: " << s.getFullName() << " | Diem TB: " << s.layDiem() << '\n';
                    found = true;
                }
            }
            if (!found) cout << "Khong tim thay sinh vien lop nay.\n";
        }
        else cout << "Lua chon khong hop le.\n";
    }
    void filterExams(QuanLyThi& thi) {
            SetColor(LIGHT_CYAN);
        cout << "\n===== LOC DE THI =====\n";
        SetColor(WHITE);
        cout << "1. Loc theo mon\n2. Loc theo loai thi\n3. Loc theo trang thai (khoa/mo)\n";
        int option = nhapSo<int>("Chon tieu chi: ");

        if (option == 1) {
            string mon = inputLine("Nhap ten mon: ");
            bool found = false;
            for (const auto& de : thi.getMaDe()) {
                if (de.tenMon.find(mon) != string::npos) {
                    cout << "ID: " << de.id << " | Mon: " << de.tenMon
                        << " | Loai: " << (de.loaiThi == 1 ? "Chinh thuc" : "On tap")
                        << " | Khoa: " << (de.locked ? "Co" : "Khong") << '\n';
                    found = true;
                }
            }
            if (!found) { SetColor(YELLOW); cout << "Khong tim thay de thi.\n"; SetColor(WHITE); }
        }
        else if (option == 2) {
            cout << "1. Chinh thuc  2. On tap\n";
            int loai = nhapSo<int>("Chon loai: ");
            if (loai != 1 && loai != 2) { SetColor(RED); cout << "Lua chon khong hop le.\n"; SetColor(WHITE); return; }
            bool found = false;
            for (const auto& de : thi.getMaDe()) {
                if (de.loaiThi == loai) {
                    cout << "ID: " << de.id << " | Mon: " << de.tenMon
            ...
            if (!found) { SetColor(YELLOW); cout << "Khong tim thay de thi loai nay.\n"; SetColor(WHITE); }
        }
        else if (option == 3) {
            cout << "1. Khoa  2. Mo\n";
            int trangThai = nhapSo<int>("Chon trang thai: ");
            if (trangThai != 1 && trangThai != 2) { SetColor(RED); cout << "Lua chon khong hop le.\n"; SetColor(WHITE); return; }
            bool found = false;
            for (const auto& de : thi.getMaDe()) {
                // SỬA LẠI ĐIỀU KIỆN IF NHƯ SAU:
                if ((trangThai == 1 && de.locked) || (trangThai == 2 && !de.locked)) {
                    cout << "ID: " << de.id << " | Mon: " << de.tenMon
                        << " | Trang thai: " << (de.locked ? "Khoa" : "Mo") << '\n';
                    found = true;
                }
            }
            if (!found) { SetColor(YELLOW); cout << "Khong tim thay de thi.\n"; SetColor(WHITE); }
    ...
    void findExamRooms() {
            cout << "\n===== TIM PHONG THI =====\n";
            cout << "1. Tim theo suc chua (toi thieu)\n2. Tim theo trang thai\n3. Hien thi tat ca\n";
            int option = nhapSo<int>("Chon tieu chi: ");

            if (option == 1) {
                int minCapacity = nhapSo<int>("Nhap suc chua toi thieu: ");
                bool found = false;
                for (const auto& phong : examRooms) {
                    if (phong.sucChua >= minCapacity) {
                        cout << "ID: " << phong.id << " | Ten: " << phong.tenPhong << " | Suc chua: " << phong.sucChua << " | Trang thai: " << (phong.locked ? "Khoa" : "Mo") << '\n';
                        found = true;
                    }
                }
                if (!found) cout << "Khong tim thay phong.\n";
            }
            else if (option == 2) {
                cout << "1. Khoa  2. Mo\n";
                int trangThai = nhapSo<int>("Chon trang thai: ");
                if (trangThai != 1 && trangThai != 2) { cout << "Lua chon khong hop le.\n"; return; }
                bool found = false;
                for (const auto& phong : examRooms) {
                    if ((trangThai == 1 && phong.locked) || (trangThai == 2 && !phong.locked)) {
                        cout << "ID: " << phong.id << " | Ten: " << phong.tenPhong << " | Suc chua: " << phong.sucChua << " | Trang thai: " << (phong.locked ? "Khoa" : "Mo") << '\n';
                        found = true;
                    }
                }
        ...
        void adminMenu(QuanLyThi & thi) {
            ...
            do {
                ...
                SetColor(LIGHT_MAGENTA);
                cout << "║ [9]  Xem thong ke he thong                 ║\n";
                cout << "║ [10] Thong ke sinh vien / giang vien       ║\n";
                cout << "║ [11] Quan ly phong thi                     ║\n";
                cout << "║ [12] Xuat bao cao thi                      ║\n";
                cout << "║ [13] Xem thong ke bao cao                  ║\n";
                ...
                cout << "║ [14] Luu du lieu                           ║\n";
                cout << "║ [15] Tai du lieu                           ║\n";
                cout << "║ [16] Tim kiem sinh vien                    ║\n";
                cout << "║ [17] Loc de thi                            ║\n";
                cout << "║ [18] Tim phong thi                         ║\n";
                ...
                case 10: countUsers(); break;
                case 11: manageExamRooms(); break;
                case 12: exportExamReport(thi); break;
                case 13: viewExamStatistics(thi); break;
                case 14:
                    saveData();
                    thi.saveDeThi();
                    break;
                case 15:
                    loadData();
                    break;
                case 16: searchStudent(); break;
                case 17: filterExams(thi); break;
                case 18: findExamRooms(); break;
